...
from transformers import AutoTokenizer, AutoModelForCausalLM, BitsAndBytesConfig, EarlyStoppingCallback, AutoConfig
...
if sample_type in ['infoverse', 'llm_search', 'selectllm']:
    run_name = f'mistral_{data_set}_{sample_type}_{ftype}_{n_instances}_rs{random_state}'
    new_output_dir = OUTPUT_DIR.joinpath(data_set, sample_type, ftype, n_instances, str(random_state))
else:
    run_name = f'mistral_{data_set}_{sample_type}_{n_instances}_rs{random_state}'
